...
ACTIVE_SWARMS=$(npx claude-flow@alpha hive-mind sessions 2>/dev/null | grep -c "Status: active" | head -1 || echo "0")
    ...
    case $action in
        1)
            ...
            if [ "$ACTIVE_SWARMS" -gt "0" ] 2>/dev/null; then
                ...
                bash "$SCRIPT_DIR/swarm-manager.sh" <<< "2"$'\n'"yes"$'\n'
            fi
        ...
        3)
            ...
            echo ""
            echo "=== Active Swarms ==="
            npx claude-flow@alpha hive-mind sessions | head -20
            echo ""
            exit 0
            ;;
        *)
...
tmux send-keys -t $SESSION_NAME:2 "sleep 10" C-m
tmux send-keys -t $SESSION_NAME:2 "echo ''" C-m
